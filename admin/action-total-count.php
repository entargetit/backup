...
if($m_data==1)
{
 ...
 $taday_mx=date('Y-m-d');
$query_select1mx="SELECT * FROM new_report where emp_name <> ''";
...
else if($m_data==2)
{
 ...
 $taday_mx=date('Y-m-d');
 $query_select1mx="SELECT * FROM new_report where emp_name <> '' AND created_date LIKE '%".$taday_mx."%'";
 $runmx=$db->select($query_select1mx);
 while($rowmx=$runmx->fetch_assoc())
 {
 $requested=$requested+$rowmx['request'];
...
else if($m_data==3)
{
 ...
 $taday_mx=date('Y-m-d');
 $query_select1mx="SELECT * FROM new_report where emp_name <> '' AND created_date between date_sub(now(),INTERVAL 7 DAY) and now()";
 $runmx=$db->select($query_select1mx);
 while($rowmx=$runmx->fetch_assoc())
 {
 $requested=$requested+$rowmx['request'];
